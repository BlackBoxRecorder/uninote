...
## 更新摘要
**变更内容**
- 媒体嵌入节点安全增强：添加沙箱属性和全屏播放支持
- 更新安全处理与访问控制章节，包含新的安全措施
- 增强嵌入内容的安全性，防止恶意脚本执行
...
VID["media-video-node.tsx<br/>含沙箱属性"]
AUD["media-audio-node.tsx"]
FIL["media-file-node.tsx"]
EMB["media-embed-node.tsx<br/>含沙箱属性"]
...
- 视频：支持 YouTube、Bilibili、本地上传播放；可调整宽高范围；**增强安全沙箱属性**
- 音频：内嵌播放控件
- 文件：下载链接与标题
- 嵌入：支持视频与推文，自动解析并生成嵌入链接；**增强安全沙箱属性和全屏播放支持**
...
- **安全增强**：Bilibili 嵌入使用沙箱属性 `allow-scripts allow-same-origin allow-presentation`，支持全屏播放
...
- 自适应宽高：根据提供商设置不同的宽高比
- 工具栏：支持编辑链接、标题与删除
- **安全增强**：所有嵌入内容使用沙箱属性 `allow-scripts allow-same-origin allow-presentation`，支持全屏播放
...
**更新** 媒体嵌入节点现已增强安全特性，包括沙箱属性和全屏播放支持，进一步提升了嵌入内容的安全性和用户体验。
...
- 上传安全
  ...
  - 对上传路径进行随机化命名，避免路径暴露
- 嵌入安全
  - **增强的沙箱保护**：所有嵌入内容使用 `allow-scripts allow-same-origin allow-presentation` 沙箱属性，限制脚本执行权限
  - 仅允许可信域名的嵌入，必要时对 iframe 进行沙箱限制
  - **全屏播放支持**：嵌入内容支持全屏播放，同时保持安全边界
- 访问控制
  - 对私有内容增加鉴权与权限检查
  - 对外链嵌入设置 referrer 策略与 CORS 控制

**更新** 媒体嵌入节点现已实现增强的安全措施，包括沙箱属性和全屏播放支持，有效防止恶意脚本执行并提升用户体验。