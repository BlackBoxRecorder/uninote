...
## 更新摘要
**变更内容**
- 新增应用状态管理中的智能标签切换逻辑和内容保存机制
- 增强编辑器状态的保存状态管理和内容缓存策略
- 新增保存确认对话框组件用于处理未保存内容的切换
- 完善标签过滤系统的智能切换逻辑
...
NOTEITEM["笔记项<br/>src/components/file-tree/note-item.tsx"]
PLATE["编辑器<br/>src/components/editor/plate-editor.tsx"]
IDEASP["想法页面<br/>src/components/ideas/ideas-page.tsx"]
TAGFILTER["标签过滤栏<br/>src/components/ideas/tag-filter-bar.tsx"]
DIARYP["日记页面<br/>src/components/diary/diary-page.tsx"]
CONFIRMDLG["保存确认对话框<br/>src/components/ui/confirm-dialog.tsx"]
end
SHELL --> FILETREE
SHELL --> DIARYP
SHELL --> IDEASP
...
**图表来源**
- [src/stores/app-store.ts:1-374](file://src/stores/app-store.ts#L1-L374)
- [src/stores/editor-store.ts:1-281](file://src/stores/editor-store.ts#L1-L281)
- [src/stores/ideas-store.ts:1-125](file://src/stores/ideas-store.ts#L1-L125)
...
- [src/components/file-tree/note-item.tsx:1-209](file://src/components/file-tree/note-item.tsx#L1-L209)
...
- [src/components/ui/confirm-dialog.tsx:1-149](file://src/components/ui/confirm-dialog.tsx#L1-L149)
...
**章节来源**
- [src/stores/app-store.ts:51-107](file://src/stores/app-store.ts#L51-L107)
...
participant IDEAS as "IdeasStore"
participant API as "后端接口"
UI->>APP : 触发标签页切换
APP->>EDIT : 检查保存状态并保存未保存内容
APP->>APP : 更新 activeTab
APP->>IDEAS : 切换到想法页面时加载标签
IDEAS->>API : 获取标签列表
API-->>IDEAS : 返回标签数据
IDEAS->>IDEAS : 加载想法列表
IDEAS->>API : 获取想法数据
...
IDEAS-->>UI : 渲染标签过滤栏
UI->>IDEAS : 用户点击标签
IDEAS->>IDEAS : setSelectedTagId
IDEAS->>IDEAS : fetchIdeas(true) 重新加载
IDEAS->>API : 获取筛选后的想法
API-->>IDEAS : 返回筛选结果
IDEAS-->>UI : 更新想法列表
```
...
**图表来源**
- [src/stores/app-store.ts:53-107](file://src/stores/app-store.ts#L53-L107)
...
- 设计要点
  ...
  - **新增** 智能标签切换逻辑：在 setActiveTab 中检查编辑器保存状态，如果当前有未保存内容则自动保存。
...
```mermaid
flowchart TD
Start(["开始标签切换"]) --> CheckSave{"编辑器有未保存内容？"}
CheckSave --> |是| AutoSave["自动保存当前内容"]
CheckSave --> |否| UpdateTab["更新标签页"]
AutoSave --> SaveSuccess{"保存成功？"}
SaveSuccess --> |是| UpdateTab
SaveSuccess --> |否| ShowError["显示保存错误"]
UpdateTab --> LoadContent["加载新标签页内容"]
LoadContent --> End(["结束"])
ShowError --> End
```

**图表来源**
- [src/stores/app-store.ts:57-61](file://src/stores/app-store.ts#L57-L61)

**章节来源**
- [src/stores/app-store.ts:51-107](file://src/stores/app-store.ts#L51-L107)
- [src/stores/app-store.ts:263-374](file://src/stores/app-store.ts#L263-L374)

### 编辑器状态（EditorStore）分析
- 设计要点
  - LRU 内容缓存：Map 存储 {content, wordCount, timestamp}，超过阈值时淘汰最久未访问项。
  - **增强** 保存状态管理：完整支持 saved/saving/unsaved/error 四种状态，提供精确的状态反馈。
...
EDIT->>EDIT : 序列化/计算字数
EDIT->>EDIT : 设置 saveStatus = 'saving'
EDIT->>API : PATCH 保存
API-->>EDIT : 返回保存结果
alt 保存成功
EDIT->>EDIT : 更新缓存与保存状态
else 保存失败
EDIT->>EDIT : 设置 saveStatus = 'error'
end
EDIT-->>UI : 渲染保存结果
```
...
### 想法状态（IdeasStore）分析
- 设计要点
  - **新增** 智能标签切换：通过 selectedTagId 控制过滤；每次筛选触发重置刷新，确保数据一致性。
...
- [src/stores/ideas-store.ts:4-18](file://src/stores/ideas-store.ts#L4-L18)
- [src/stores/ideas-store.ts:29-59](file://src/stores/ideas-store.ts#L29-L59)
- [src/stores/ideas-store.ts:73-125](file://src/stores/ideas-store.ts#L73-L125)
...
AppStore --> IdeasStore : "切换标签页时加载标签"
...
- 内部依赖
  ...
  - AppStore 依赖 IdeasStore（切换标签页时加载标签）。
...
FILETREE --> EDIT
NOTEITEM["NoteItem"] --> EDIT
NOTEITEM --> CONFIRMDLG["SaveConfirmDialog"]
DIARYP["DiaryPage"] --> DIARY
DIARYP --> EDIT
IDEASP["IdeasPage"] --> IDEAS
IDEASP --> TAGFILTER["TagFilterBar"]
```
...
- [src/components/file-tree/note-item.tsx:10-10](file://src/components/file-tree/note-item.tsx#L10-L10)
...
- [src/components/ideas/tag-filter-bar.tsx:3-3](file://src/components/ideas/tag-filter-bar.tsx#L3-L3)
- [src/components/ui/confirm-dialog.tsx:1-1](file://src/components/ui/confirm-dialog.tsx#L1-L1)
...
- **新增** 智能保存机制
  - 应用状态中的自动保存逻辑避免了用户手动保存的负担，提升了用户体验。
  - 保存状态的精确管理提供了更好的用户反馈。
...
- 常见问题
  ...
  - **新增** 保存状态异常：检查 saveStatus 是否正确更新，确认保存流程中的错误处理。
- 调试建议
  - 在开发环境开启严格模式与 React DevTools，观察组件重渲染次数。
  - 对关键状态（如 contentCache、diaryEntries、selectedTagId）进行断点或日志输出，验证更新路径。
- 性能监控
...
## 结论
YNote v2 的状态管理以 Zustand 为核心，结合清晰的功能域划分与严格的原子性/不可变性原则，实现了高可维护性的前端状态体系。通过 LRU 缓存、乐观更新、细粒度订阅与序列化优化，兼顾了性能与用户体验。**新增的智能标签切换逻辑和内容保存机制进一步提升了用户体验，确保数据的一致性和安全性。** 未来可在以下方面持续演进：引入更完善的调试工具链、增强类型安全与边界校验、探索状态快照与时间旅行调试能力。

## 附录
- 类型定义概览
  - 文件夹、笔记、想法、日记等核心实体类型集中于类型文件，确保 Store 与 UI 的契约清晰。
  - **新增** SaveStatus 类型定义，支持完整的保存状态管理。
- 组件与 Store 的绑定
  - AppShell 作为壳层协调三个功能区；FileTree、DiaryPage、IdeasPage 分别驱动对应 Store 的初始化与刷新。
  - **新增** SaveConfirmDialog 组件提供保存确认对话框，处理未保存内容的切换场景。
...
- [src/components/ui/confirm-dialog.tsx:98-149](file://src/components/ui/confirm-dialog.tsx#L98-L149)