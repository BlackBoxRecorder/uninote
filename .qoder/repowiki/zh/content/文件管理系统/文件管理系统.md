...
- [src/stores/editor-store.ts](file://src/stores/editor-store)
...
## 简介
本文件管理系统围绕“文件树”组件构建，提供递归渲染与动态加载能力；支持文件夹的创建、删除、重命名与归档/取消归档；支持笔记的创建、重命名、删除与下载（Markdown）；提供基于 Drizzle ORM 的 SQLite 数据库存储；通过统一的存储抽象层支持本地或云端存储；提供搜索、上下文菜单、确认对话框等用户体验优化；并具备与数据库的同步机制。
...
- 笔记项组件 NoteItem：支持重命名、下载（Markdown）、导入到飞书文档、删除；带保存确认对话框与提示。
...
- [src/components/file-tree/note-item.tsx:24-219](file://src/components/file-tree/note-item.tsx#L24-L219)
...
图表来源
- [src/components/file-tree/note-item.tsx:52-98](file://src/components/file-tree/note-item.tsx#L52-L98)
- [src/stores/editor-store.ts](file://src/stores/editor-store.ts)
- [src/app/api/notes/route.ts:42-85](file://src/app/api/notes/route.ts#L42-L85)
...
- 下载
  - 笔记下载：NoteItem 组件直接跳转到 /api/notes/[id]/download。
  - 静态文件：/api/files/[...path] 路由读取 data/uploads 目录下的文件，防止目录穿越，按扩展名映射 MIME 类型。
- 错误处理
  - 上传失败时重置进度。
  - 下载/导入异常通过 toast 提示。
...
章节来源
- [src/hooks/use-upload-file.ts:10-52](file://src/hooks/use-upload-file.ts#L10-L52)
- [src/components/file-tree/note-item.tsx:92-94](file://src/components/file-tree/note-item.tsx#L92-L94)
- [src/app/api/files/[...path]/route.ts:7-47](file://src/app/api/files/[...path]/route.ts#L7-L47)
...
- 预览建议
  ...
  - 对于 Markdown 文档，NoteItem 已支持下载为 Markdown。
...
- [src/components/file-tree/note-item.tsx:151-161](file://src/components/file-tree/note-item.tsx#L151-L161)
- [src/components/file-tree/note-item.tsx:200-206](file://src/components/file-tree/note-item.tsx#L200-L206)
...
- 上传失败
  ...
  - 查看浏览器网络面板与控制台错误日志。
- 下载失败
  - 确认 /api/notes/[id]/download 是否返回 200；检查笔记是否存在。
